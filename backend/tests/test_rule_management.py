...
Updated to use clause_number and Chinese risk_level values (低风险/中风险/高风险).
"""
import pytest
...
class TestRuleGroupCRUD:
    ...
    def test_get_rule_groups_empty(self, client: TestClient):
        ...
        response = client.get("/api/v1/rule-groups")
        assert response.status_code == 200
    ...
    def test_get_rule_groups(self, client: TestClient):
        ...
        # Create two groups
        client.post("/api/v1/rule-groups", json={"name": "Group 1"})
        client.post("/api/v1/rule-groups", json={"name": "Group 2"})
    ...
    def test_get_rule_group_by_id(self, client: TestClient):
        ...
        # Get by ID
        response = client.get(f"/api/v1/rule-groups/{group_id}")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_rule_group_not_found(self, client: TestClient):
        ...
        response = client.get("/api/v1/rule-groups/non-existent-id")
    ...
    def test_update_rule_group_not_found(self, client: TestClient):
        ...
        response = client.put(
            "/api/v1/rule-groups/non-existent-id",
            json={"name": "New Name"}
        )
        assert response.status_code == 404
    ...
    def test_delete_rule_group(self, client: TestClient):
        ...
        # Create a group
        create_response = client.post("/api/v1/rule-groups", json={"name": "To Delete"})
        group_id = create_response.json()["id"]

        # Delete it
        response = client.delete(f"/api/v1/rule-groups/{group_id}")
        assert response.status_code == 200
        # assert response.json()["message"] == "Rule group deleted"

        # Verify it's gone
        get_response = client.get(f"/api/v1/rule-groups/{group_id}")
        assert get_response.status_code == 404

    def test_delete_rule_group_not_found(self, client: TestClient):
        """Test deleting a non-existent rule group."""
        response = client.delete("/api/v1/rule-groups/non-existent-id")
        assert response.status_code == 404
...
class TestRuleCRUD:
    ...
    def test_create_rule(self, client: TestClient):
        ...
        group_response = client.post("/api/v1/rule-groups", json={"name": "Test Group"})
    ...
    def test_create_rule_minimal(self, client: TestClient):
        ...
        assert data["risk_level"] == "中风险"  # Default value

    def test_create_rule_invalid_risk_level(self, client: TestClient):
        """Test creating a rule with invalid risk_level value."""
        group_response = client.post("/api/v1/rule-groups", json={"name": "Test Group"})
        group_id = group_response.json()["id"]

        response = client.post(
            f"/api/v1/rule-groups/{group_id}/rules",
            json={"clause_number": "1.0", "content": "Test", "risk_level": "InvalidValue"}
        )
        assert response.status_code == 400
        assert "risk_level" in response.json()["detail"].lower()
    ...
    def test_create_rule_group_not_found(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/rule-groups/non-existent-id/rules",
    ...
    def test_get_rules(self, client: TestClient):
        ...
        # Create rules
        client.post(f"/api/v1/rule-groups/{group_id}/rules", json={"clause_number": "1.0", "content": "Rule 1"})
        client.post(f"/api/v1/rule-groups/{group_id}/rules", json={"clause_number": "2.0", "content": "Rule 2"})

        # Get rules
        response = client.get(f"/api/v1/rule-groups/{group_id}/rules")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2
    ...
    def test_get_rules_empty(self, client: TestClient):
        ...
        group_response = client.post("/api/v1/rule-groups", json={"name": "Empty Group"})
        group_id = group_response.json()["id"]
    ...
    def test_update_rule(self, client: TestClient):
        ...
        rule_response = client.post(
            f"/api/v1/rule-groups/{group_id}/rules",
            json={"clause_number": "1.0", "content": "Original Content", "risk_level": "中风险"}
        )
        ...
        response = client.put(
            ...
            json={"content": "Updated Content", "risk_level": "高风险"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_delete_rule(self, client: TestClient):
        ...
        # Verify it's gone
        rules_response = client.get(f"/api/v1/rule-groups/{group_id}/rules")
    ...
    def test_delete_group_cascades_rules(self, client: TestClient):
        ...
        # Delete group
        client.delete(f"/api/v1/rule-groups/{group_id}")
...
class TestCSVImportExport:
    ...
    def test_export_csv(self, client: TestClient):
        ...
        # Create group with rules
        group_response = client.post("/api/v1/rule-groups", json={"name": "Export Test"})
        group_id = group_response.json()["id"]

        client.post(f"/api/v1/rule-groups/{group_id}/rules", json={
            "clause_number": "1.0",
        ...
        # Export CSV
        response = client.get(f"/api/v1/rule-groups/{group_id}/export-csv")
        assert response.status_code == 200
    ...
    def test_import_csv(self, client: TestClient):
        ...
        group_response = client.post("/api/v1/rule-groups", json={"name": "Import Test"})
        group_id = group_response.json()["id"]

        # Create CSV content
        csv_content = """id,standard_name,clause_number,content,review_type,risk_level
,测试标准,1.1,第二条规则内容,内容完整性,高风险
,测试标准,1.2,第二条规则内容,禁止条款,中风险"""

        # Import CSV
        response = client.post(
            f"/api/v1/rule-groups/{group_id}/import-csv",
