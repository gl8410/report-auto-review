      ...
      <div className="max-w-3xl mx-auto mt-8 bg-white p-8 rounded-xl shadow-lg border border-slate-200">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-slate-900 mb-2">
            {isCompleted ? '✅ 审查完成' : isFailed ? '❌ 审查失败' : '🔍 审查进行中'}
          </h2>
          <p className="text-slate-500">
            {isCompleted ? '所有规则已审查完毕' : isFailed ? '审查过程中出现错误' : 'AI正在逐条审查规则...'}
          </p>
        </div>
          ...
          <div className="flex mb-2 items-center justify-between">
            <div className="flex items-center gap-2">
              <span className={`text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full ${
                ...
                {status === 'PROCESSING' ? '处理中' : status === 'PENDING' ? '等待中' : status === 'COMPLETED' ? '已完成' : '失败'}
              </span>
        ...
        {/* Real-time Stats */}
        {(status === 'PROCESSING' || isCompleted) && stats && (
